    ...
    const list = async () => {
      try {
        ...
        if (location.pathname === `/tag/${slug}`) {
          ...
          setPanigation(posts.data.metaData.pagination);
          dispatch(setLoading(false));
          ...
          setPanigation(questions.data.metaData.pagination);
          dispatch(setLoading(false));
          ...
          setPanigation(followers.data.metaData.pagination);
          dispatch(setLoading(false));
        }
      } catch (error) {
        dispatch(setLoading(false));
        ...
        <div className="col-start-1 col-span-3 w-full mb-[20px] rounded">
          <div className="flex py-[30px] px-[10px] bg-white rounded mb-[15px]">
              ...
              {tag?.avatar?.avatarUrl ? (
                ...
                <div className="w-[150px] h-[150px] bg-blue-400 flex justify-center items-center rounded-[5px]">
                  <p className="text-white text-[32px]">
                    {tag?.name?.toUpperCase().substring(0, 1)}
                  </p>
                </div>
              )}
            </div>
            <div className="ml-[30px] my-auto">
              <div className="">
                <h3 className="text-[30px] font-semibold inline-block">
                  {tag?.name}
                </h3>
          ...
          <div className="w-full shadow-sm bg-white rounded">
            ...
            <Switch>
              <Route
                exact
                path={path.TAGS_ID}
                render={(props) => <PostInTag data={posts} {...props} />}
              ></Route>
              <Route
                exact
