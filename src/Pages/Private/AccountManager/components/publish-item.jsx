...
const PublishItem = (props) => {
  ...
  const { id, index, fullname, username, email, status, role, type, reCall } =
    props;
  ...
  const changeStatus = async (e, username) => {
    try {
      ...
      await UserApi.editManagerUser(username, dataSend);
  ...
  return (
    <div className="nav bg-white border-b text-[15px] p-[10px] grid grid-cols-[60px,1.5fr,1.5fr,1.5fr,0.6fr,0.6fr,0.8fr]">
      ...
      <div>
        <select
          disabled={profile.username === username}
          onChange={(e) => handleChangeRole(e, username)}
          defaultValue={role}
          className="border py-[5px] px-[8px] border-green-300 rounded"
          name="role"
        >
